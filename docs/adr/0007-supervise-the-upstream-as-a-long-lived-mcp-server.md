...
- **守护进程是每账户一个，且同一时刻只认一个 cache 目录。** 我们把 cache 指到 `<root>/.knowledge-base/cbm`（索引应当和它描述的根目录放在一起），代价是同一个 OS 账户下不能有第二处 CBM 用法：只要活着的守护进程持有的 cache 与请求的不一致，上游连 `config set` 都会直接拒绝，报 `active cache <hash>; requested cache <hash>`。因此服务要跑在专用的 `knowledge-base` 账户下；用真实二进制的测试也无法与正在运行的服务共存，只能先停服务（测试会检测到这种拒绝并跳过，而不是伪装成代码缺陷）。`ExecStopPost` 同样要带上一致的 `CBM_CACHE_DIR`。
